...
public class MyCommentsFragment extends BaseFragment {
    ...
    public View onCreateView(LayoutInflater inflater, @Nullable ViewGroup container, @Nullable Bundle savedInstanceState) {
        ...
        if ( null == mContentView ) {
            ...
            adapter = new MyCommentAdapter(getActivity(),this);
    ...
    private void loadData(final boolean isMore) {
        ...
        CommonAppModel.myComment(req, new HttpResultListener<MyCommentResponseVo>() {
            @Override
            public void onSuccess(MyCommentResponseVo resp) {
                manager.showLoadSuccess();
                swipyrefreshlayout.setRefreshing(false);
                if (resp.isSuccess()) {
                    List<CommentShowVo> temp = resp.getCommentShowVoArr();
                    if (temp.size() > 0) {
